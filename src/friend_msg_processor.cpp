...
void FriendMsgProcessor(FriendMessage m)
{
	try
	{
		...
		if (cmds[0] == "!通知:" && senderQQ == ADMIN_QQ)
		{
			...
			for (auto user : users)
			{
				try
				{
					bot.SendMessage(user, m.MessageChain);
				}
				catch (const std::exception& ex)
				{
					cout << ex.what() << endl;
